#!/usr/bin/env pwsh
# install.ps1 - Install infra tool for Windows
...
if ($Uninstall) {
    ...
    if ($UserPath -like "*$BinDir*") {
        ...
        Write-Host "infra bin directory was not found in PATH." -ForegroundColor Yellow
    }
} else {
    # Add to PATH
    if ($UserPath -like "*$BinDir*") {
        Write-Host "infra bin directory is already in PATH." -ForegroundColor Yellow
    } else {
...
if (-not $Uninstall) {
    ...
    Write-Host "  infra --help" -ForegroundColor White
    Write-Host "  infra setup ." -ForegroundColor White
    Write-Host ""
