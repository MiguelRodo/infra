#!/usr/bin/env pwsh
# infra.ps1 - Project infrastructure setup utility wrapper for Windows
...
$SubcommandScripts = @{
    "setup"         = "setup.sh"
    "install-repos" = "install-repos.sh"
}
...
if (-not (Test-Path $TargetScript)) {
    ...
    Write-Error "The infra package may not be installed correctly."
    exit 1
}
